...
function formatCurrency(amount) {
    ...
    return 'P' + num.toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,');
}
...
// Store the previous event listener to remove it later
let previousProductClickListener = null;
...
function setupProductEventListeners() {
    ...
    if (previousProductClickListener) {
        productsContainer.removeEventListener("click", previousProductClickListener);
    }

    // Define the event listener as a named function
    const handleProductClick = (event) => {
        console.log("Click event fired"); // Debug: Check if this logs multiple times
        ...
        const price = parseFloat(productBox.querySelector("p").textContent.split("P")[1]);
    ...
    previousProductClickListener = handleProductClick;
...
function exportInventoryReport() {
    ...
    doc.text(`Generated Time: ${now.toLocaleTimeString()}`, 14, 36);
    ...
    doc.text(`Filters: ${filters}`, 14, 43);
    
    // Add summary section
    doc.setFontSize(12);
    doc.text("Inventory Summary", 14, 52);
    ...
    doc.text(`Low Stock Items: ${lowStockCount}`, 14, 69);
    doc.text(`Out of Stock Items: ${outOfStockCount}`, 14, 76);
    doc.text(`Expiring Soon (≤7 days): ${expiringSoonCount}`, 14, 83);
...
function exportSalesReport() {
    ...
    // Generate timestamp in YYYY-MM-DD-HH-MM-SS format (e.g., "2025-07-25-22-00-00")
    const now = new Date();
    const timestamp = [
        now.getFullYear(),
        String(now.getMonth() + 1).padStart(2, '0'),
    ...
    // Base filename (e.g., "sales_report_daily_2025-07-25-22-00-00")
    let fileName = `sales_report_${period}_${timestamp}`;
    ...
    filtered.forEach(sale => {
        sale.items.forEach(item => {
            const profit = item.unitPrice * item.quantity; 
            tableData.push([
